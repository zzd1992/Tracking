...
def get_axis_aligned_BB(bbox):
    width = bbox.shape[1]    
    x_axis = np.arange(0,width,2,dtype='int32')
    y_axis = np.arange(1,width,2,dtype='int32')
    cx = np.mean(bbox[:,x_axis],axis=1)
    cy = np.mean(bbox[:,y_axis],axis=1)
    x1 = np.min(bbox[:,x_axis],axis=1)
    x2 = np.max(bbox[:,x_axis],axis=1)
    y1 = np.min(bbox[:,y_axis],axis=1)
    y2 = np.max(bbox[:,y_axis],axis=1)
    A1 = np.sqrt(np.sum((bbox[:,:2]-bbox[:,2:4])**2,axis=1))*np.sqrt(np.sum((bbox[:,2:4]-bbox[:,4:6])**2,axis=1))
    A2 = (x2 - x1) * (y2 - y1)
    s = np.sqrt(A1/A2)
    w = s * (x2 - x1) + 1
    h = s * (y2 - y1) + 1
    bbox_rect = np.zeros((bbox.shape[0],4),dtype='float32')
    bbox_rect[:,0] = cx
    bbox_rect[:,1] = cy
    bbox_rect[:,2] = w
    bbox_rect[:,3] = h
